...
22.9.0, localhost HTTP и WKWebView подтверждены на физическом iPhone. Stage 3
включает официальный SillyTavern 1.18.0 и проходит локальный полный
`--jitless`-тест, но новая IPA ещё не проверена на устройстве.
...
WebAssembly в текущем V8 отключён. Для Stage 3 несовместимые стартовые пути
заменены: HTTP-запросы используют `node-fetch`, Webpack исполняется в CI,
Jimp использует JS-кодеки PNG/JPEG, а tiktoken заменён приблизительным
байтовым токенизатором. WASM-функции Transformers/ONNX, WebP и AVIF остаются
неподдержанными.

Репозиторий находится на честном Stage 3 candidate: настоящий SillyTavern
запускается локально, но готовность будет объявлена только после device-теста.
...
SwiftUI
  ...
  └── Objective-C++ bridge
        └── node_start()
              ...
              └── Node Worker
                    └── SillyTavern 1.18.0 (127.0.0.1, 8000+)
```
...
- stop/restart реализованы завершением и повторным созданием Worker при
  сохранении control server и основного Node event loop;
- `process.exit()` внутри SillyTavern завершает только Worker, а не SwiftUI-хост.
...
### Stage 3: Node 22 и SillyTavern

`build-node22-and-ipa.yml` использует device slice из закреплённого коммита
Node 22.9.0 port, подготавливает официальный tag SillyTavern 1.18.0 и создаёт
unsigned IPA. Node runtime и payload кэшируются независимо. Симулятор не
участвует в workflow, потому что для установки через LiveContainer он не нужен.

Локальные патчи исправляют сборку host-инструментов на macOS и обновляют
устаревший список библиотек framework: удалённые в Node 22 `libbase64*`
исключаются, а `nbytes`, `ncrypto`, `sqlite`, `simdjson` и новые архивы V8
явно добавляются к линковке.
...
Полный on-device update manager сознательно не включён в первую Stage 3 IPA.
Он будет добавлен после device-подтверждения настоящего SillyTavern.
...
- Node 22 device slice и device-only XCFramework собираются;
