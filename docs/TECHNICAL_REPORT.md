...
`build-node22-and-ipa.yml` собирает device slice из закреплённого коммита
Node 22.9.0 port, создаёт device-only XCFramework и unsigned IPA. Симулятор
не участвует в этом проверочном workflow, потому что для установки на iPhone
он не требуется, а полная вторая сборка Node/V8 удваивает время и расход диска.
Закрепление защищает CI от незаметного изменения выполняемых build scripts.
